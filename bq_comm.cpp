...
void BQ79656::Comm(RequestType req_type, byte data_size, byte dev_addr, RegisterAddress reg_addr, byte *data)
{
    ...
    uint16_t command_crc =
        crc.Modbus(bqBuf, 0, isStackOrBroad ? 4 : 5);  // calculates the CRC, but the bytes are backwards
...
bool BQ79656::verifyCRC(uint8_t *buf) { return crc.Modbus(buf, 0, bqBufDataLen) == 0; }

uint8_t *BQ79656::GetBuf() { return bqBuf; }
...
void BQ79656::GetTemps(float *temps)
{
    ...
    for (int i = 1; i <= stackSize; i++)
    {
        for (int j = 0; j < thermoPerSegment; j++)
        {
            ...
            temps[((i - 1) * thermoPerSegment) + j] =
                temp[0] * (152.59 * 0.000001);  // Result * V_LSB_GPIO to get voltage
